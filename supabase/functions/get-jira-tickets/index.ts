...
Deno.serve(async (req)=>{
  ...
  // Prepare request body for new POST endpoint
  const requestBody = {
    jql: jqlQuery,
  ...
  // Add pagination token if startAt > 0 (note: this is a simplified approach)
  if (startAt > 0) {
    // For backward compatibility, we'll still use startAt logic
    // In a full migration, you'd need to implement proper token-based pagination
    requestBody.startAt = startAt;
  }
  
  console.log("POSTING TO: " + `${jira_base_url}/rest/api/3/search/jql`);
