...
interface DeleteMindspaceFileRequest {
...
serve(async (req) => {
  ...
  try {
    ...
    const request: DeleteMindspaceFileRequest = await req.json();
    ...
    if (isBulkDelete) {
      return new Response(
        JSON.stringify({
          ...
          message: `Deleted ${successCount} of ${fileIds.length} files`,
    ...
    console.error('Error in delete-mindspace-file function:', error);
