...
COMMENT ON FUNCTION public.get_agno_sessions (
    ...
    INTEGER,
    INTEGER
) IS 'Returns paginated sessions with internal message pagination (msg_page 1 = most recent messages). Messages with same created_at timestamp are ordered by msg_ordinality to ensure deterministic ordering.';
